 ...
 */
@DownloadEngine(systemType = "HP-KPI", measType = "CM", operatorName = "VODAFONE")
public class HpKpiDownloadEngine extends DefaultTimeBasedEngine {

    public HpKpiDownloadEngine(ServerIpList eachIp) {
        super(eachIp);
    }
    ...
    public void onDownlaod(Connection con, ServerIpList connectionInfo, List<RemoteFileObject> fileList) {
        ...
        for (RemoteFileObject each : fileList) {
            //NeName Filter
            try {
                if (!functionSubsetNames.contains(each.getFileName().split("\\_", 3)[2].split("\\.")[0].trim())) {
                    continue;
                }
            } catch (Exception e) {
                continue;
            ...
            if (each.getDate().after(getLastDateFromdb())) {
                String localFilePath = DownloadApp.LOCALFILEPATH + each.getFileName();
                ...
                try {
                    fileDate = new SimpleDateFormat("yyyyMMddHHmmss").parse(each.getFileName().split("\\_")[1]);
