 ...
 */
@DownloadEngine(systemType = "FTTS", measType = "CM", operatorName = "VODAFONE")
public class FttsDownloadEngine extends CopyEngine {

    public FttsDownloadEngine(ServerIpList eachIp) {
        super(eachIp);
    }
        ...
        for (LocalFileObject each : files) {
            ...
            try {
                ...
                for (String fileName : fileNames) {
                    if (each.getFileName().startsWith(fileName)) {
                        fileFilter = true;
                        break;
                    }
                }
            ...
            if (each.getDate().after(getLastDateFromdb())) {
                ...
                try {
                    fileDate = new SimpleDateFormat("yyyy-MM-dd").parse(each.getFileName().substring(each.getFileName().length() - 17, each.getFileName().length() - 9));
